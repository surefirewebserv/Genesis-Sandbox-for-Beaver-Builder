 ...
 */
function genesis_starter_customizer_get_default_link_color() {
	return '#c3251d';
}
 ...
 */
function genesis_starter_customizer_get_default_accent_color() {
	return '#c3251d';
}
/**
 ...
 * 
 */
function genesis_starter_customizer_get_default_header_color() {
	return '#ffffff';
}

function genesis_starter_customizer_get_default_header_opacity() {
	return '1';
}
 ...
 */
function genesis_starter_customizer_register() {
...
/*----------------------------------------------------------------------------------------------------*/
	...
	$wp_customize->add_setting( 
		...
		array(
			'default' 			=> 0,
			'capability' 		=> 'edit_theme_options',
			'sanitize_callback' => 'genesis_starter_checkbox_sanitize'
		)
	);
...
// Checkbox Sanitize
function genesis_starter_checkbox_sanitize( $input ) {
	if ( $input == 1 ) {
		return 1;
	} else {
		return '';
...
if( $primary_nav_responsive == 'normal' ){
	function genesis_starter_enqueue_responsive_menu() {
	...
	add_action( 'wp_enqueue_scripts', 'genesis_starter_enqueue_responsive_menu' );
}
if( $primary_nav_responsive != 'normal' ){
	add_action( 'genesis_before', 'genesis_starter_responsive_menu'); 
	function genesis_starter_responsive_menu(){
		?>
...
add_action( 'wp_enqueue_scripts', 'genesis_starter_css' );
/**
...
function genesis_starter_css() {

	$handle  = defined( 'CHILD_THEME_NAME' ) && CHILD_THEME_NAME ? sanitize_title_with_dashes( CHILD_THEME_NAME ) : 'child-theme';

	$color_link = get_theme_mod( 'genesis_starter_link_color', genesis_starter_customizer_get_default_link_color() );
	$color_accent = get_theme_mod( 'genesis_starter_accent_color', genesis_starter_customizer_get_default_accent_color() );

	$css = '';

	//* Calculate Color Contrast
	function genesis_starter_color_contrast( $color ) {
	...
	function genesis_starter_color_brightness( $color, $change ) {
	...
	$css .= ( genesis_starter_customizer_get_default_link_color() !== $color_link ) ? sprintf( '
	...
	$css .= ( genesis_starter_customizer_get_default_accent_color() !== $color_accent ) ? sprintf( '
		...
		', $color_accent, genesis_starter_color_contrast( $color_accent ) ) : '';
